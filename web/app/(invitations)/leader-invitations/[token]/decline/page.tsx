...
import { resolveLeaderInvitation, declineLeaderInvitation } from '@/lib/api/invitations'
...
export default function DeclineInvitationPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  async function handleDecline() {
    ...
    try {
      await declineLeaderInvitation(token)
  ...
  if (pageState === 'already_accepted') return <AlreadyAcceptedState orgName={invitation?.organization.name ?? 'the organization'} />
  ...
  const workshopTitle = inv.workshop?.title ?? null
  const workshopDates =
    inv.workshop
      ? (() => {
          const start = new Date(inv.workshop.start_date)
          const end = new Date(inv.workshop.end_date)
  ...
  return (
    <div style={cardStyle}>
      ...
      <p style={{ fontFamily: 'var(--font-jakarta), Plus Jakarta Sans, sans-serif', fontSize: '14px', color: '#6B7280', lineHeight: 1.6, marginBottom: workshopDates ? '4px' : '24px' }}>
        {inv.organization.name} invited you to lead sessions{workshopTitle ? ` for "${workshopTitle}"` : ''}.
      </p>

      {workshopDates && (
        <p style={{ fontFamily: 'var(--font-jakarta), Plus Jakarta Sans, sans-serif', fontSize: '13px', color: '#9CA3AF', marginBottom: '24px' }}>
          {workshopDates}
        </p>
      )}
      ...
      <div className="flex flex-col" style={{ gap: '12px' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          <Link
            href={`/leader-invitations/${token}/accept`}
