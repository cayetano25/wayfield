...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('order_items', function (Blueprint $table) {
            ...
            $table->foreign('applied_tier_id', 'order_items_applied_tier_fk')
                ->references('id')
                ->on('workshop_price_tiers')
                ->nullOnDelete();

            $table->index('applied_tier_id', 'idx_order_items_tier');
        });
    }
    ...
    {
        Schema::table('order_items', function (Blueprint $table) {
            $table->dropForeign('order_items_applied_tier_fk');
            $table->dropIndex('idx_order_items_tier');
